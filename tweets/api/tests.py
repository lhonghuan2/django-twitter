...
class TweetApiTests(TestCase):
    ...
    def test_create_with_files(self):
        ...
        file1 = SimpleUploadedFile(
            name='selfile1.jpg',
            content=str.encode('selfile 1'),
            content_type='image/jpeg',
        )
        file2 = SimpleUploadedFile(
            name='selfile2.jpg',
            content=str.encode('selfile 2'),
            content_type='image/jpeg',
        )
        response = self.user1_client.post(TWEET_CREATE_API, {
            'content': 'two selfiles',
            'files': [file1, file2],
        })
        self.assertEqual(response.status_code, 201)
        ...
        self.assertEqual('selfile1' in response.data['photo_urls'][0], True)
        self.assertEqual('selfile2' in response.data['photo_urls'][1], True)

        # uploading more than 9 files will be failed
        files = [
            SimpleUploadedFile(
                name=f'selfile{i}.jpg',
                content=str.encode(f'selfile{i}'),
                content_type='image/jpeg',
            )
            for i in range(10)
        ]
